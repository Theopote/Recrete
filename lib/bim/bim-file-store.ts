...
export async function listBimModelsFromFile(projectId: string): Promise<BimModel[]> {
  const models = await readManifest(projectId);
  return models.sort((a, b) => b.createdAt.getTime() - a.createdAt.getTime());
}

export async function getBimModelFromFile(
  projectId: string,
  modelId: string
...
export async function addBimModelToFile(model: BimModel): Promise<BimModel> {
...
export async function updateBimModelInFile(
  projectId: string,
  modelId: string,
  patch: Partial<
...
export async function deleteBimModelFromFile(projectId: string, modelId: string): Promise<boolean> {
  const models = await readManifest(projectId);
