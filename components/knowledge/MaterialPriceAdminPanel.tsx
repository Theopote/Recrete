...
export function MaterialPriceAdminPanel() {
  ...
  const handleDelete = async (id: string) => {
    if (!confirm("Delete this material price record?")) return;
  ...
  const handleReset = async () => {
    if (!confirm("Reset all material prices to default seed data?")) return;
    setResetting(true);
    try {
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="grid grid-cols-2 md:grid-cols-4 gap-3">
        ...
        <Card>
          <CardContent className="p-4">
            <p className="text-[10px] text-muted-foreground uppercase tracking-wide">Avg Trend</p>
